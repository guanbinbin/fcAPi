 ...
 class Base extends Controller {
    ...
    public function addBase(){
        ...
        if(empty($house_name)) {
            ...
            if(!empty($house_subway_line)) {
              $baseInfo['house_subway_line'] = $house_subway_line;
            };
    ...
    */ 
    public function deleteBase(){
        ...
        $res = Db::name('house_base')->delete($house_id);

        if(!empty($res)) {
            $data = Array("code"=>200,"msg"=>"删除成功！","data"=>$res,"success"=>true);
        }else {
            $data = Array("code"=>0,"msg"=>"删除失败！","data"=>0,"success"=>false);
        }
        return json($data);
    }
    ...
    *   @params house_id 
    */ 
    public function getOneBase(){
        $request = Request::instance();
        $house_id =  $request->param('house_id');

        $data = Db::name('house_base')->where('house_id',$house_id)->select();
    ...
    public function updateBase(){
        ...
        if(empty($house_name)) {
            ...
            //客户更新日期
            $baseInfo['add_date'] = date("Y-m-d");
            $res = Db::name('house_base')->update($baseInfo);
